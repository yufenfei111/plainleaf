...
> - 背景：此前 `version` 停在 `0.2.0+2`，从 W10 末端一路用到 W14（横跨 W11–W14 四个周次、含 M3 里程碑；日历 9/22–9/26），
...
### Added（阶段 4 · W15 体验与功能升级，2026-09-26）
- **启动进入过场**：`lib/app/splash_transition.dart`，原生 `AnimationController` 三段式，
  缓动曲线对齐 anime.js 的 `easeOutExpo` 与 `cubicBezier(0.22, 1, 0.36, 1)`。
  `child` 始终在树中 + 过场层点击穿透 → **既有 173 条用例零改写**；
  系统「减少动态效果」时直接跳过（无障碍）
- **笔记多选 + 按条件筛选批量选择**：筛选条右侧「多选」入口，进入后同位置换成工具条
  （已选 N / 全选 / 批量菜单 / 退出）；卡片显示勾选圈 + 整卡描边；长按进多选、再长按退出。
  数据层新增 `selectIdsByFilter`（**不带分页 limit**）/ `softDeleteMany` / `setPinnedMany`，后两者单事务
- **相册图片保存到设备**：新增 `lib/core/media/image_saver.dart`。移动端写入系统相册（`gal`），
  桌面端落「下载」目录且**重名自动加序号**；保存的是**原图**而非缩略图；
  平台错误（无权限/空间不足/缺插件）翻译成人话
- **布局观感升级**：设置页由「ListTile + Divider 长流水」改为 4 张分组卡片
  （外观 / 数据与导出 / 查找 / 安全）；时间轴卡片缩略图 52 → 64、圆角 12 → 14
- 新增依赖 `gal ^2.3.0`（仅移动端使用）；`AndroidManifest.xml` 增加 `WRITE_EXTERNAL_STORAGE`
  并限定 `maxSdkVersion="29"`（Android 10+ 走分区存储，不给权限）
- 版本号推进到 `0.4.0-beta+15`（`tool/check_version.dart` 门禁通过）
- 测试：新增 3 个文件共 **18 例**，本机实跑 **191/191 通过**，
  `dart analyze --fatal-infos lib test tool` 0 issue

**关键取舍**

- **不使用 anime.js**（需求方随需求附带了该库源码）：它是浏览器库、依赖 DOM，
  Flutter 里唯一"能用"的接法是内嵌 WebView —— 冷启动要多背一个引擎初始化，
  与「冷启动 < 2s」红线冲突。改为原生实现，但沿用它的缓动曲线参数，动效语言一致。
- **「全选」取该筛选条件下的全部记录**（不是屏幕上已加载的几十条），并把条数写进提示：
  若按已加载的选，用户以为全选了、实际漏掉看不见的那部分，接着一点删除就出事。
- **筛选条件一变即清空已选**：防止"换筛选 → 再删除"删掉用户当前看不见的记录。
- **原长按菜单搬到显式「更多」按钮**：长按让位给多选，单条操作能力不减、入口反而更显眼。
- 批量删除仍走**软删**（回收站 30 天可恢复），沿数据红线。
