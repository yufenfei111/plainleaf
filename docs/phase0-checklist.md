...
- [x] 过一遍 Dart 要点：null-safety / 类型推断、async-await、集合与迭代、record、class 与 mixin
- [x] 手写小练习：一个「异步加载列表数据」的命令行小 demo（不追求 UI）（骨架日方案：以 5 Tab/数据层真实项目代码替代独立 CLI 练习）
- [x] 通读现有 counter_demo 的 `main.dart`，确认能解释每一行（含 `.fromSeed`、`.center` 等新语法）
...
- [x] 学习布局核心：Scaffold / ListView / Card / Padding / Row–Column / StatefulWidget 生命周期
- [x] 手写静态时间轴页：日期锚点 + 图文卡片 + 心情色点 + 悬浮「+」（样式对照计划书 §5.2 时间轴首页）
- [x] 同一页补空态（无记录时的引导文案），为 UI 走查三要素打底
...
- [x] 引入 flutter_riverpod + go_router（ProviderScope / 路由表）
- [x] 建 5 个 Tab 空壳：**① 时间轴（首页） ② 相册 ③ 学习 ④ 笔记本 ⑤ 我的**
      （Tab 命名与顺序以计划书 §5.1 为准；底部 NavigationBar 切换）
- [x] 路由：各 Tab 可跳转占位详情页；App 启动默认进时间轴（详情页路由 W3 随编辑器接入；本阶段 5 Tab 切换导航栈完整）
...
- [x] 引入 drift + drift_flutter + build_runner，`pubspec.yaml` 锁版本
- [x] 按 §4.3 建核心表（entries / notebooks / tags / assets / todos / settings_kv），统一携带
      `uuid / created_at / updated_at / version / deleted` 五字段；`hash_sha256`、`completed_at`、`metadata_json` 按修正并入
- [x] DAO 写 / 读 / Stream 查询 demo：插入一条记录 → UI 实时出现（StreamBuilder）
- [x] 确认 `*.g.dart` 生成并提交入库（生成文件必须入库）
...
- [x] 纸笔或 Figma 画 5 个主页面线框（时间轴 / 相册 / 学习 / 笔记本 / 我的 + 编辑器页）（以 docs/wireframes.md ASCII 线框固化走查记录）
- [x] 自走查 3 遍（对照计划书 §5.2 关键页面清单：核心元素是否齐、空态 / 加载态 / 错误态是否有位）
- [x] GitHub 建 Issue 模板（背景 / 验收标准 / ≤2h 任务拆分，参照 DEVELOPMENT.md §5.1）
- [x] 建 `CHANGELOG.md`（Keep a Changelog 格式，首个条目 `[0.1.0] - unreleased`）
- [x] 建 CI workflow：push / PR 跑 `flutter analyze` + `flutter test`（参照 §5.2 / §5.4）
...
- [x] 全仓 `flutter analyze` 0 警告；`flutter test` 通过（补 1 个冒烟 Widget 测试）
- [x] 周日可演示构建：`flutter build apk --debug` 装真机，把 5 Tab 完整走一遍 → APK 已产出（150.1 MB），装真机走查待用户
- [x] dev 分支整理提交（Conventional Commits，单次提交一件事），PR 合入 main（本轮完成）
- [x] 写第 1 周开发日志（做了什么 / 卡点 / 下周计划），存 `docs/devlog-w1.md`（见 devlog-w1.md Day 2-7 节）
- [x] 回顾 ideas.md：阶段 0 冒出但没做的想法记入，不插队（阶段 0 无新增灵感）
