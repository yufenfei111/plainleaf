...
> **2026-09-26 校准**：以下 6 项与 `verification-w13.md` 的坚果云清单已合并整理为
> **可直接照着点的走查手册** —— `docs/device-checklist-w13-w14.md`
> （补齐真实 UI 文案、逐项失败判据与排查对照）。**走查以手册为准，本节保留作历史记录。**
...
4. 加密包恢复全流程：本地列表的标题**后缀**显示「 · 已加密」（`settings_page.dart:997`，
   无 🔒 图标——此处 2026-09-26 按源码校准），选中后弹密码框，密码错提示
   「密码不正确，或者这份数据已被改动过」；W13 导出的明文老包仍能正常恢复。
   注意：**应用锁**的错密码文案是「密码不正确」（`lock_screen.dart`），与上面这句 GCM 长文案不是同一句。
